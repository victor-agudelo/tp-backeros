    ...
    async def create_restaurant(
        ...
        if current_user.rol != "admin":
            raise InvalidRequest(
                status_code=401, detail="No tiene permisos para crear un restaurante")
        ...
        if not propietario_restaurante:
            raise InvalidRequest(
                status_code=401, detail="El propietario no existe")
        if propietario_restaurante.rol != "propietario":
            raise InvalidRequest(
                status_code=401, detail="El rol no es de propietario")

        new_restaurant = FullRestaurant(
            **restaurant_info.dict(), idRestaurante=propietario_restaurante.idUsuario)
