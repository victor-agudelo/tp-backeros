...
class OrderServices:
    ...
    async def update_current_order(self, orderId, estado, empleado=None):
        ...
        if not current_order:
            raise InvalidRequest(
                status_code=401, detail="No existe esta orden")

        if current_order.get("estado") == "Cancelada":
            raise InvalidRequest(
                detail="No es posible actualizar el estado de una orden cancelada")
    ...
    async def finish_order(self, order_id, code):
        ...
        if current_order.get('estado') != "Listo":
            raise InvalidRequest(
                status_code=401, detail="El pedido aun no esta Listo")
